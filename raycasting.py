...
class RayCasting:
    ...
    def raycast(self):  # Refactorizado extract methods
        ...
        for ray in range(NUM_RAYS):  # Calculamos los ángulos de cada rayo
            ...
            distancia_vert, textura_vert, y_vert = self.interseccion_lineas_verticales(cos_a, ox, oy, sin_a,
                                                                                       textura_vert, x_mapa)

            # Determinamos la coordenada de intersección con las líneas horizontales
            distancia_hor, textura_hor, x_hor = self.interseccion_lineas_horizontales(cos_a, ox, oy, sin_a, textura_hor,
                                                                                      y_mapa)

            # Elegimos la distancia final entre vertical u horizontal y la textura adecuada
            distancia, offset, textura = self.elegir_distancia_final(angulo_ray, cos_a, distancia_hor, distancia_vert,
                                                                     sin_a, textura_hor, textura_vert, x_hor, y_vert)
    ...
    def elegir_distancia_final(self, angulo_ray, cos_a, distancia_hor, distancia_vert, sin_a, textura_hor, textura_vert,
                               x_hor, y_vert):

        """ Método parte del raycast para elegir la distancia final del rayo y su textura correspondiente """
        ...
        return distancia, offset, textura

    def interseccion_lineas_horizontales(self, cos_a, ox, oy, sin_a, textura_hor, y_mapa):
        """ Método parte del raycast para determinar la distancia a una pared horizontal del rayo """
        if sin_a > 0:
            y_hor, dy = (y_mapa + 1, 1)
        else:
        ...
        return distancia_hor, textura_hor, x_hor

    def interseccion_lineas_verticales(self, cos_a, ox, oy, sin_a, textura_vert, x_mapa):
        """ Método parte del raycast para determinar la distancia a una pared vertical del rayo """
        if cos_a > 0:
        ...
        return distancia_vert, textura_vert, y_vert
