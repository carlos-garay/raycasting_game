...
class Llave(Coleccionable):
    ...
    def logica_coleccionar(self):
        ...
        if distancia < self.flyweight.rango_coleccion and distancia != 0 and self.bandera_activo:
            ...
            self.flyweight.juego.sonido.llave.play()
            self.flyweight.juego.jugador.llave = True

            # SUBSTITUTE ALGORITHM
            # Cambiamos la textura de la puerta a que no tenga candado
            posicion_puerta = self.flyweight.juego.mapa.posicion_puerta
            self.flyweight.juego.mapa.mapa_mundo[posicion_puerta] = 4
